...
from asr_project.noise_augment import create_noisy_benchmark, profile_from_modulo
...
def main():
    parser = argparse.ArgumentParser(description="Run Part D (noise + transcription + evaluation).")
    parser.add_argument(
        "--musan_root",
        required=True,
        help="Path to extracted MUSAN root (containing music/noise/speech folders).",
    )
    parser.add_argument(
        "--modulo_val",
        type=int,
        default=2,
        help="Modulo value in [0..5] that selects the SNR/profile group.",
    )
    parser.add_argument(
        "--seed",
        type=int,
        default=42,
        help="Random seed for noise mixing.",
    )
    ...
    cfg.musan_root = Path(args.musan_root)
    if not cfg.musan_root.exists():
        raise FileNotFoundError(f"MUSAN path does not exist: {cfg.musan_root}")
    musan_wav_count = len(list(cfg.musan_root.rglob("*.wav")))
    if musan_wav_count == 0:
        raise RuntimeError(f"No .wav files found under MUSAN path: {cfg.musan_root}")
    ...
    profile = profile_from_modulo(args.modulo_val)
    ...
    print()
    print(f"MUSAN wav files found: {musan_wav_count}")
    print(f"Noise log: {part_d_noise_log_tsv}")
